...
optimizer = torch.optim.Adam(model.parameters(), lr=2e-3, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-5, amsgrad=True)
...
for period in range(1,no_periods+1):
    ...
    t.learning_phase(no_frame_samples, batch_size, input_window_size, first_frame,
                     last_frame,frame_skip*2,criterion,optimizer,device,learning=1,num_epochs=40000)
